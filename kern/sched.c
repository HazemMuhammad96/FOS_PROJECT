...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	unsigned int envReadyQueueSize = numOfLevels * sizeof(struct Env_Queue);
	unsigned int qunatumsSize = numOfLevels * sizeof(uint8);

	env_ready_queues = kmalloc(envReadyQueueSize);
	quantums = kmalloc(qunatumsSize);
...
{
	int queueLevel = 0;
	while (queueLevel < num_of_ready_queues)
	{
		int queueProcesses = queue_size(&(env_ready_queues[queueLevel]));
		if (queueProcesses > 0)
		{
...
struct Env *fos_scheduler_MLFQ()
{
	...
	if (curenv != NULL)
	{
		if (currentQueueLevel >= num_of_ready_queues)
			enqueue(&(env_ready_queues[num_of_ready_queues - 1]), curenv);
		else
		{
			enqueue(&(env_ready_queues[currentQueueLevel]), curenv);
			currentQueueLevel += 1;
		}
	}
	...
	if (findQueue())
	{
		kclock_set_quantum(quantums[currentQueueLevel]);
		nextEnv = dequeue(&(env_ready_queues[currentQueueLevel]));
	}
